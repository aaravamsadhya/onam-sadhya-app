...
// The WhatsApp message templates below deliberately avoid colorful/pictographic emoji (the
// wheat-sheaf, hibiscus, curry-rice, blossom, coconut, keycap-number, warning, checkmark, cherry
// blossom, canoe style of emoji). Confirmed directly against WhatsApp's own api.whatsapp.com/
// wa.me link handler: it silently mangles those into "?" replacement characters on WhatsApp's own
// servers before the message ever reaches a chat - this happens even when the text arriving at
// WhatsApp is byte-perfect UTF-8, so it cannot be fixed from our side. Simple single-codepoint
// symbols (checkmark, cross, arrows, stars) and WhatsApp's own *bold*/_italic_ markdown syntax
...
app.get('/api/admin/share-coupons', async (req, res) => {
  ...
  try {
    ...
    msg += '1. Open your respective coupon link\n';
    msg += '2. Select your preferred entry slot to reserve your seat\n';
    msg += '3. Download a copy of your coupon to your device (only after selecting the preferred slot) — this is mandatory and required for entry\n\n';
...
app.get('/api/admin/share-reminder', async (req, res) => {
  ...
  try {
    ...
    coupons.forEach((c, i) => {
      const status = c.slotNumber ? (SYM.check + ' Slot booked: Slot ' + c.slotNumber + ' - ' + c.slotTime) : (SYM.cross + ' No slot booked yet');
      msg += (i + 1) + '. *' + c.name + '* (' + c.type + ') — ' + status + '\n' + c.url + '\n';
    });
    msg += '\nNext steps:\n';
    msg += '1. Open your coupon link\n';
    msg += '2. Select your preferred entry slot to reserve your seat\n';
    msg += '3. Download a copy of your coupon to your device (only after selecting the preferred slot) — this is mandatory and required for entry\n\n';
